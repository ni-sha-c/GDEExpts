...
def plot_attractor(optim_name, num_epoch, lr, time_step):
    ...
    if time_step == 5e-4:
        ...
        tran = 2000 
        X, Y, X_test, Y_test = sol.create_data(0, 180, 
                                torch.Tensor([ -8., 7., 27.]), 120*2000+1, 
                                n_train=10000, n_test=7500, n_nodes=3, n_trans=tran)
...
##### run experiment #####    
plot_attractor('AdamW', 20000, 5e-4, 1e-2) # optimizer name, epoch, lr, time_step